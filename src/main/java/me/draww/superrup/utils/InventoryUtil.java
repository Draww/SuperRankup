 ...
 * @author Acrobot
 * @edited Draww
 * Original https://github.com/ChestShop-authors/ChestShop-3/blob/master/src/main/java/com/Acrobot/Breeze/Utils/InventoryUtil.java
 */
public class InventoryUtil {
    private static Boolean legacyContents = null;
    private static Boolean STACK_TO_64 = true;
    ...
    public static boolean hasItems(ItemStack[] items, Inventory inventory) {
        ...
        for (ItemStack item : mergedItems) {
            int getA = getAmount(item, inventory);
            if (getA < item.getAmount()) {
                return false;
            }
        }
    ...
    public static int getMaxStackSize(ItemStack item) {
        return STACK_TO_64 ? 64 : item.getMaxStackSize();
    }
